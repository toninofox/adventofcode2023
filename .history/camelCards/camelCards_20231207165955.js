...
const sameOfAKind = (cards) => 
    Object.values(cards.filter(c => c !== "J" ).reduce((acc, card) => {
        if (acc[card]) {
            acc[card]++
        } else {
            acc[card] = 1
...
const getRank = (cards) => {
    ...
    if (jmostOccorrung === 5 || jokers === 5) {
        return 7
    } else if (jmostOccorrung === 4) {
        return 6
    } else if (jmostOccorrung === 3 && secondMostoccorring === 2) {
        return 5
    } else if (jmostOccorrung === 3) {
        return 4
    } else if (jmostOccorrung === 2 && secondMostoccorring === 2) {
        return 3
...
const play = ()=> {
    const hands =input.jsample.split("\n").map(r=>{
