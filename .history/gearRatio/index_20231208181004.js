...
const sum = ()=>{
    ...
    lines.forEach((line, i) => {
       const numbers = getNumbers(line);
        ...
        console.log(numbers, prevSymb, currentSymb.map(c=>lines[i-1].charAt(c)), nextSymb, numbers.filter(n=>isValidRange(prevSymb, n, line) || isValidRange(currentSymb, n, line) || isValidRange(nextSymb, n,line)).map(v=>parseInt(v.v)))
        numbers.filter(n=> isValidRange(prevSymb, n,line)|| isValidRange(currentSymb, n,line)|| isValidRange(nextSymb, n,line)).map(v=>parseInt(v.v)).forEach(r=> res+=r)
    });
    console.log(res)
}
sumM()
