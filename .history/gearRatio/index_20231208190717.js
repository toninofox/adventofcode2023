...
const getValidGearRange = (numbers, g, line) => numbers.filter(n => {
    const endRow = g.i > 0 ? g.i - n.v.length : line.length - 1;
    const startRow = g.i < line.length ? g.i + 1 : 0;
    return (g.i === endRow || g.i === startRow) || (g.i >= startRange && g.i <= endRange)
})
...
const sum2 = () => {
    ...
    lines.forEach((line, i) => {
        ...
        if (gears[0])
            console.log(
                gears, 
                getValidGearRange(prevNumbers, gears[0], line),
