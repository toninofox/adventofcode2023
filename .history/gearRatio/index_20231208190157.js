...
const isValidGearRange = (numbers,gear,line)=>numbers.some(n=>{
    const endRow = g.i > 0 ? -1 : line.length - 1;
    const startRow = g.i < line.length ? -1 : 0;
    const startRange = g.i - n.v.length
    const endRange = g.i + 1;
    return (gear.i === endRow || gear.i === startRow) || (gear.i >= startRange && gear.i <= endRange)
}
...
const sum2 = () => {
    ...
    lines.forEach((line, i) => {
        ...
        if(gears[0])
        console.log(
            gears,prevNumbers.filter(n=>isValidSIngleRage(n.i,gears[0],line)),
            currentNumbers.filter(n => isValidSIngleRage(n.i, gears[0], line)) ,
            nextNumbers.filter(n => isValidSIngleRage(n.i, gears[0], line)))
    })
}
sum2()
